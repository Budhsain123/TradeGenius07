# main.py - Fixed Referral System with Username-based Channel Addition
...
✅ Channel addition with @username FIXED
"""
    ...
    def show_verification_screen_real_time(self, chat_id, user_id, username):
        ...
        for channel_id, channel in channels.items():
            ...
            if channel_link.startswith("@"):
                ...
                # If just username without @, add it
                if not channel_link.startswith("@"):
                    channel_link = "@" + channel_link
                channel_url = f"https://t.me/{channel_link[1:]}"
...
✅ <b>Naya Format (आसान तरीका):</b>
सिर्फ @ के साथ चैनल का यूजरनेम भेजें:

<code>@TradeGenius07_HelpCenter_bot</code>

<b>या पुराना फॉर्मेट:</b>
<code>Channel Name
@channel_username
channel_id</code>
...
You can now request withdrawals."""
                ...
                # ✅ NEW: Check if it's just a username (starts with @)
                if text.startswith("@"):
                    # Single username format
                    channel_username = text
                    channel_name = channel_username[1:].replace("_", " ").title()
                    
                    # Create channel data
                    channel_data = {
                        "name": channel_name,
                        "link": channel_username,  # Store as @username
                        "id": channel_username,    # Use username as ID
                    ...
                    if len(lines) >= 3:
                        ...
                        # Ensure channel link has @ prefix
                        if channel_link and not channel_link.startswith("@"):
                            channel_link = "@" + channel_link
                        ...
                        msg = "❌ Invalid format. Send just @username or full 3-line format."
...
We're here to help!"""
    ...
    def run_bot(self):
        ...
        print("✅ FIXED: Channel Addition with @username")
...
if __name__ == "__main__":
    print("🔥 Trade Genius Bot - Fixed Channel System")
    print("="*50)
    print("✅ Now you can add channels with just @username")
    print("✅ No need to send full link or channel ID")
    print("="*50)
